...
struct node
{
    ...
    node(T key, std::vector<U> value, ul id) :
...
public:
    ...
    void insert(std::vector<T>& t, std::vector< std::vector<U> >& u)
    {
        ...
        for (typename std::vector<T>::size_type i = 0; i < t.size(); ++i)
        {
            ...
            typename H<T, U>::handle_type h =
                heap_.push(node<T, U>(t[i], u[i], id_));
            h->handle = h;

            id_to_handles_.insert(std::pair<ul, typename H<T, U>::handle_type>(id_, h));
...
private:
    ...
    std::unordered_map<ul, typename H<T, U>::handle_type> id_to_handles_;
